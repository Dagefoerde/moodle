...
class repository_nextcloud extends repository {
    ...
    public function __construct($repositoryid, $context = SYSCONTEXTID, $options = array()) {
        ...
        if (!$this->issuer) {
            ...
            $this->disabled = true;
            return;
        } else if (!\repository_nextcloud\issuer_management::is_valid_issuer($this->issuer)) {
    ...
    public function reference_file_selected($reference, $context, $component, $filearea, $itemid) {
        ...
        $linkmanager = new \repository_nextcloud\access_controlled_link_manager($this->ocsclient, $this->issuer, $this->get_name());
    ...
    public function send_file($storedfile, $lifetime=null , $filter=0, $forcedownload=false, array $options = null) {
        ...
        $linkmanager = new \repository_nextcloud\access_controlled_link_manager($this->ocsclient, $this->issuer, $repositoryname);
        ...
        redirect($webdavurl, get_string('downloadpopup', 'repository_nextcloud',
     ...
     * @throws \repository_nextcloud\configuration_exception if an endpoint is undefined
     */
    private function parse_endpoint_url($endpointname) {
        $url = $this->issuer->get_endpoint_url($endpointname);
        if (empty($url)) {
            throw new \repository_nextcloud\configuration_exception(sprintf('Endpoint %s not defined.', $endpointname));
        }
